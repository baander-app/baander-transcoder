...
export class TranscodeConfig {
  static transcodePath({encoderId, sha, segmentName, indexOrQuality}: TranscodePathArgs): string {
    ...
    return path.join(TranscodeConfig.baseTranscodePath(), sha, `${segmentName}-${encoderId}-${indexOrQuality}-%d.ts`);
  }
